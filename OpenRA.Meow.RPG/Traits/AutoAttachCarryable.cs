...
{
	[Desc("Can be carried by units with the trait `AttachCarryall`.")]
	public class AutoAttachCarryableInfo : AttachCarryableInfo
	{
		...
		public override object Create(ActorInitializer init) { return new AutoAttachCarryable(this); }
	}
	...
	public class AutoAttachCarryable : AttachCarryable, ICallForTransport
	{
		...
		public AutoAttachCarryable(AutoAttachCarryableInfo info)
			: base(info)
		{
			this.info = info;
		}
		...
		void RequestTransport(CPos destination)
		{
			...
			var carriers = Self.World.ActorsWithTrait<AutoAttachCarryall>()
				.Where(c => c.Trait.EnableAutoCarry && c.Trait.State == AttachCarryall.AttachCarryallState.Idle && !c.Actor.IsDead && c.Actor.Owner == Self.Owner && c.Actor.IsInWorld)
