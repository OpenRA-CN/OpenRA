...
then
	...
	then
		TITLE=$(zenity --forms --add-combo="" --combo-values="Red Alert 2|Tiberian Sun" --text "Select mod" --title="" || echo "cancel")
		if [ "$TITLE" = "cancel" ]; then exit 0
		elif [ "$TITLE" = "Tiberian Sun" ]; then MODARG='Game.Mod=ts'
		else MODARG='Game.Mod=ra2'
		fi
	else
		echo "Please provide the Game.Mod=\$MOD argument (possible \$MOD values: ra2, ts)"
		exit 1
	fi
fi
