		...
		public IngameChatLogic(Widget widget, OrderManager orderManager, World world, ModData modData, bool isMenuChat, Dictionary<string, MiniYaml> logicArgs)
		{
			...
			chatText.OnEnterKey = _ =>
			{
				...
				if (chatText.Text != "")
				{
					...
					else if (chatText.Text.StartsWith("fxaa"))
					{
						Game.Settings.Graphics.FxAA = !Game.Settings.Graphics.FxAA;
						orderManager.IssueOrder(Order.Chat("FxAA: " + Game.Settings.Graphics.FxAA));
					}
