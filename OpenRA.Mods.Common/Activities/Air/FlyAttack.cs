...
namespace OpenRA.Mods.Common.Activities
{
	public class FlyAttack : Activity, IActivityNotifyStanceChanged
	{
		...
		public override bool Tick(Actor self)
		{
			...
			if (rearmable != null && !useLastVisibleTarget && attackAircraft.Armaments.All(x => x.IsTraitPaused || !x.WeaponIsValidAgainst(target, self.World, self)))
			{
			...
			else if (attackAircraft.Info.FacingFullOnTarget || !attackAircraft.TargetInFiringArc(self, target, attackAircraft.Info.FacingTolerance))
			{
