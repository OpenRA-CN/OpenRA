...
namespace OpenRA.Mods.Common.Graphics
{
	public class IsometricSelectionBoxAnnotationRenderable : IRenderable, IFinalizedRenderable
	{
		...
		public void Render(WorldRenderer wr)
		{
			...
			for (var i = 0; i < 6; i++)
			{
				cr.DrawScreenLine(new float3[] { screen[i] + Offsets[3 * i], screen[i], screen[i] + Offsets[3 * i + 1] }, 1, color, true);
				cr.DrawScreenLine(new float3[] { screen[i], screen[i] + Offsets[3 * i + 2] }, 1, color, true);
			}
		}
