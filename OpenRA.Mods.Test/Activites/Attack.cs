...
namespace OpenRA.Mods.TA.Activities
{
	/* non-turreted attack */
	public class Attack : Activity, IActivityNotifyStanceChanged
	{
		...
		readonly IEnumerable<AttackInfantry> attackTraits;
		...
		public Attack(Actor self, in Target target, bool allowMovement, bool forceAttack, Color? targetLineColor = null)
		{
			...
			attackTraits = self.TraitsImplementing<AttackInfantry>().ToArray().Where(Exts.IsTraitEnabled);
		...
		protected virtual AttackStatus TickAttack(Actor self, AttackInfantry attack)
		{
		...
		protected virtual void DoAttack(Actor self, AttackInfantry attack, IEnumerable<Armament> armaments)
		{
